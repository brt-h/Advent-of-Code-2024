...
#                     break
#             if swap_location != -1:
#                 compact_disk_map[swap_location] = swap
#                 compact_disk_map[i] = '.'
#     return compact_disk_map

# define more efficient function to convert the verbose disk map to a compact disk map
def compact_format_efficient(verbose_disk_map):
    compact_disk_map = verbose_disk_map.copy()
    left = 0
    right = len(compact_disk_map) - 1
    while left < right:
        if compact_disk_map[left] == '.':
            if compact_disk_map[right] != '.':
                compact_disk_map[left] = compact_disk_map[right]
                compact_disk_map[right] = '.'
            else:
                right -= 1
        else:
            left += 1
...
compact_disk_map = compact_format_efficient(verbose_disk_map)
